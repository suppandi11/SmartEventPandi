...
if (isset($_POST['ubah'])) {
    ...
    $query = "UPDATE events SET judul='$judul', kategori='$kategori', tanggal='$tanggal', waktu='$waktu', lokasi='$lokasi', deskripsi='$deskripsi', kuota='$kuota' WHERE id=$id";
    if (mysqli_query($conn, $query)) { header("Location: admin.php"); exit; }
}

$kategori_list = ['Seminar','Workshop','Lomba','Pelatihan','Webinar','Pameran','Kuliah Umum','Budaya','Olahraga','Organisasi','Lainnya'];
if (!in_array($data['kategori'], $kategori_list)) {
...
<head>
    ...
    <title>Ubah Event - Smart Event</title>
...
<body class="text-[var(--ink)] min-h-screen flex flex-col">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex-1 w-full">
        ...
        <h2 class="text-2xl sm:text-[28px] font-display font-semibold text-white tracking-tight leading-none mb-6">Ubah Data Kegiatan</h2>
        ...
        <div class="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-5 sm:p-7 shadow-[0_2px_10px_rgba(0,0,0,0.35)]">
            <form action="" method="POST" class="space-y-5">
                <div>
                    ...
                    <input type="text" name="judul" value="<?= htmlspecialchars($data['judul']) ?>" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] placeholder:text-[var(--slate-soft)] focus:outline-none focus:border-[var(--gold)]">
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="lokasi" value="<?= htmlspecialchars($data['lokasi']) ?>" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] focus:outline-none focus:border-[var(--gold)]">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-[var(--gold-soft)] mb-1.5">Tanggal</label>
                        <input type="date" name="tanggal" value="<?= $data['tanggal'] ?>" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] focus:outline-none focus:border-[var(--gold)]">
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-[var(--gold-soft)] mb-1.5">Waktu</label>
                        <input type="time" name="waktu" value="<?= $data['waktu'] ?>" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] focus:outline-none focus:border-[var(--gold)]">
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wide text-[var(--gold-soft)] mb-1.5">Kuota Peserta</label>
                        <input type="number" name="kuota" value="<?= $data['kuota'] ?>" min="1" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] focus:outline-none focus:border-[var(--gold)]">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wide text-[var(--gold-soft)] mb-1.5">Deskripsi</label>
                    <textarea name="deskripsi" rows="4" required class="w-full px-4 py-2.5 border border-[var(--line)] bg-[var(--bg-2)] rounded-xl text-sm text-[var(--ink)] placeholder:text-[var(--slate-soft)] focus:outline-none focus:border-[var(--gold)] resize-none"><?= htmlspecialchars($data['deskripsi']) ?></textarea>
                </div>
                <div class="flex gap-3 justify-end pt-5 border-t border-[var(--line-soft)]">
                    ...
                    <button type="submit" name="ubah" class="px-5 py-2.5 bg-[var(--maroon)] hover:bg-[var(--maroon-2)] text-white rounded-xl font-bold text-xs cursor-pointer transition shadow-md shadow-black/40 border-b-2 border-[var(--gold)]">
                        <i class="fa-solid fa-floppy-disk mr-1"></i> Perbarui Event
                    </button>
                </div>
            </form>
        </div>
